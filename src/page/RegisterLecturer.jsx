...
const RegisterLecturer = () => {
  ...
  // Show password strength hints as user types
  const handlePasswordChange = (e) => {
    ...
    if (newPassword) {
      ...
      if (!validation.isValid) {
        setPasswordStrengthHint(validation.errors[0]); // Show first error
      } else {
  ...
  const handleRegister = async (e) => {
    ...
    const validation = validateRegistrationForm(normalizedData);
    if (!validation.isValid) {
    ...
    try {
      ...
      // Link auth_user_id for RLS enforcement
